...
add_params(Params, Key, VidiunObject) when is_tuple(VidiunObject) ->
	PropList = vidiun_object_to_proplist(VidiunObject),
	Value = add_params([], PropList),
	add_params(Params, [{Key, Value}]);
add_params(Params, Key, Value) ->
	add_params(Params, [{Key, Value}]).

-spec request(ClientConfiguration::#vidiun_configuration{}, ClientRequest::#vidiun_request{}, Params::body()) -> Response::response().
request(ClientConfiguration, ClientRequest, Params) ->
	Params1 = add_params(Params, vidiun_request_to_proplist(ClientRequest)),
...
request(ClientConfiguration, Body) ->
	Url = ClientConfiguration#vidiun_configuration.url,
    Headers = [{"Accept", "application/json"}, {"Content-Type", "application/json"}],
    Request = get_request(Url, Headers,  Body),
    httpc:set_options(ClientConfiguration#vidiun_configuration.client_options),
    Response = parse_response(httpc:request(post, Request, ClientConfiguration#vidiun_configuration.request_options, [{body_format, binary}])),
	case Response of
		{ok, _, _, Results} ->
			Results;
...
parse_response({ok, {_, Headers, Body}}) ->
	SessionId = proplists:get_value("x-vidiun-session", Headers),
