...
cp -r . $PREFIX
cd $PREFIX
sed -i "s#@BASEDIR@#$PREFIX#g" vidcliAliases.sh vidcliAutoComplete logToCli
...
if [ `id -u` = 0 ] ;then 
    ln -sf $PREFIX/vidcliAutoComplete /etc/bash_completion.d/
    ln -sf $PREFIX/vidcliAliases.sh /etc/profile.d/
fi
