...
class Php53ClientGenerator extends ClientGeneratorFromXml
{
	...
	private function cacheEnum(DOMElement $enumNode)
	{
		...
		if($enumNode->hasAttribute('plugin'))
		{
			...
			$classInfo->setNamespace("Vidiun\\Client\\Plugin\\{$pluginName}\\Enum");
		}
		else
		{
			$classInfo->setNamespace("Vidiun\\Client\\Enum");
		}
	...
	private function cacheType(DOMElement $classNode)
	{
		...
		if($classNode->hasAttribute('plugin'))
		{
			...
			$classInfo->setNamespace("Vidiun\\Client\\Plugin\\{$pluginName}\\Type");
		}
		else
		{
			$classInfo->setNamespace("Vidiun\\Client\\Type");	
		}
	...
	function generate() 
	{
		...
		$this->appendLine(' */');
		$this->appendLine('namespace Vidiun\Client;');
		$this->appendLine();
		...
		$this->appendLine("		'VidiunApiExceptionArg' => 'Vidiun\\Client\\ApiExceptionArg',");
		foreach($classNodes as $classNode)
		{
			$vidiunType = $classNode->getAttribute('name');
			$zendType = $this->getTypeClassInfo($vidiunType);
			$this->appendLine("		'$vidiunType' => '{$zendType->getFullyQualifiedNameNoPrefixSlash()}',");
		}
		$this->appendLine('	);');
		$this->appendLine('	');
	...
	protected function getEnumPath(DOMElement $enumNode)
	{
		...
		if(!$enumNode->hasAttribute('plugin'))
			return "library/Vidiun/Client/Enum/{$enumName}.php";

		$pluginName = ucfirst($enumNode->getAttribute('plugin'));
		return "library/Vidiun/Client/Plugin/{$pluginName}/Enum/{$enumName}.php";
	}
	...
	protected function getTypePath(DOMElement $classNode)
	{
		...
		if(!$classNode->hasAttribute('plugin'))
			return "library/Vidiun/Client/Type/{$className}.php";

		$pluginName = ucfirst($classNode->getAttribute('plugin'));
		return "library/Vidiun/Client/Plugin/{$pluginName}/Type/{$className}.php";
	}
	...
	protected function getServicePath($serviceNode)
	{
		...
		if(!$serviceNode->hasAttribute('plugin'))
			return "library/Vidiun/Client/Service/{$serviceName}Service.php";

		$pluginName = ucfirst($serviceNode->getAttribute('plugin'));
		return "library/Vidiun/Client/Plugin/{$pluginName}/Service/{$serviceName}Service.php";
	}
	
	protected function getPluginPath($pluginName)
	{
		$pluginName = ucfirst($pluginName);
		return "library/Vidiun/Client/Plugin/{$pluginName}/".$this->getPluginClass($pluginName).".php";
	}
	
	protected function getMainPath()
	{
		return 'library/Vidiun/Client/Client.php';
	}
	
	protected function getMapPath()
	{
		return 'library/Vidiun/Client/TypeMap.php';
	}
	...
	function writePlugin(DOMElement $pluginNode)
	{
		...
		$this->appendLine(' * @namespace');
		$this->appendLine(' */');
		$this->appendLine("namespace Vidiun\\Client\\Plugin\\".ucfirst($pluginName).";");
		$this->appendLine();
		...
		$this->appendLine("class $pluginClassName extends \Vidiun\Client\Plugin");
		$this->appendLine('{');
		...
		$this->appendLine('	protected function __construct(\Vidiun\Client\Client $client)');
		$this->appendLine('	{');
		...
		$this->appendLine('	 */');
		$this->appendLine('	public static function get(\Vidiun\Client\Client $client)');
		$this->appendLine('	{');
		$this->appendLine("		return new $pluginClassName(\$client);");
		$this->appendLine('	}');
		$this->appendLine('');
		$this->appendLine('	/**');
		$this->appendLine('	 * @return array<\Vidiun\Client\ServiceBase>');
		$this->appendLine('	 */');
		...
		foreach($serviceNodes as $serviceNode)
		{
			...
			$this->appendLine("	/**");
			$this->appendLine("	 * @return \\Vidiun\\Client\\Plugin\\".ucfirst($pluginName)."\\$serviceRelativeClassName");
			$this->appendLine("	 */");
	...
	function writeEnum(DOMElement $enumNode)
	{
		...
		if($this->generateDocs)
		{
	 	...
	 	$this->appendLine("class {$enumClassInfo->getClassName()} extends \Vidiun\Client\EnumBase");		
		$this->appendLine("{");
	...
	{
		$vidiunType = $classNode->getAttribute('name');
		if(!$this->shouldIncludeType($vidiunType))
			return;
		...
		$type = $this->getTypeClassInfo($vidiunType);
		...
		// class definition
		$baseClass = '\Vidiun\Client\ObjectBase';
		...
		foreach($classNode->childNodes as $propertyNode)
		{
			...
			{
				case "file" :
					VidiunLog::info("File attribute [$propName] are not supported for class [$vidiunType]");
					return;
				...
				case "array" :
					...
					$this->appendLine("			else");
					$this->appendLine("				\$this->$propName = \Vidiun\Client\ParseUtils::unmarshalArray(\$xml->$propName, \"$arrayType\");");
					$this->appendLine("		}");
					break;
				...
				case "map" :
					...
					$this->appendLine("				\$this->$propName = array();");
					$this->appendLine("			else");
					$this->appendLine("				\$this->$propName = \Vidiun\Client\ParseUtils::unmarshalMap(\$xml->$propName, \"$arrayType\");");
					$this->appendLine("		}");
					break;
					
				default : // sub object
					$this->appendLine("		if(count(\$xml->{$propName}) && !empty(\$xml->{$propName}))");
					$this->appendLine("			\$this->$propName = \Vidiun\Client\ParseUtils::unmarshalObject(\$xml->$propName, \"{$propType}\");");
					break;
			}
		}
	...
	function writeService(DOMElement $serviceNode)
	{
		...
		$this->appendLine(' */');
		if ($plugin)
			$this->appendLine("namespace Vidiun\\Client\\Plugin\\".ucfirst($plugin)."\\Service;");
		else
			$this->appendLine('namespace Vidiun\Client\Service;');
		$this->appendLine();
		...
		$this->appendLine("class $serviceClassName extends \Vidiun\Client\ServiceBase");
		$this->appendLine("{");
		$this->appendLine("	function __construct(\\Vidiun\\Client\\Client \$client = null)");
		$this->appendLine("	{");
		$this->appendLine("		parent::__construct(\$client);");
		$this->appendLine("	}");
	...
	function writeAction($serviceId, $serviceName, DOMElement $actionNode, $plugin = null)
	{
	    ...
	    else
	    {
			...
			switch($resultType)
			{
				...
				case 'int':
					$this->appendLine("		\$resultObject = (int)\\Vidiun\\Client\\ParseUtils::unmarshalSimpleType(\$resultXmlObject->result);");
					break;
				case 'bool':
					$this->appendLine("		\$resultObject = (bool)\\Vidiun\\Client\\ParseUtils::unmarshalSimpleType(\$resultXmlObject->result);");
					break;
				case 'bigint':
				case 'string':
					$this->appendLine("		\$resultObject = (String)\\Vidiun\\Client\\ParseUtils::unmarshalSimpleType(\$resultXmlObject->result);");
					break;
				case 'array':
					$this->appendLine("		\$resultObject = \\Vidiun\\Client\\ParseUtils::unmarshalArray(\$resultXmlObject->result, \"$arrayObjectType\");");
				...
				default:
					if ($resultType)
					{
						...
						$this->appendLine("		\$resultObject = \\Vidiun\\Client\\ParseUtils::unmarshalObject(\$resultXmlObject->result, \"{$resultType}\");");
	...
	function writeMainClient(DOMNodeList $serviceNodes, DOMNodeList $configurationNodes)
	{
		...
		foreach($serviceNodes as $serviceNode)
		{
			...
			$serviceName = $serviceNode->getAttribute("name");
			$serviceClassName = "\\Vidiun\\Client\\Service\\".ucfirst($serviceName)."Service";
			$this->appendLine("	/**");
		...
		$this->appendLine("	 *");
		$this->appendLine("	 * @param \\Vidiun\\Client\\Configuration \$config");
		$this->appendLine("	 */");
